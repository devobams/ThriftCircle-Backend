...
- **~~Payout-week contribution exemption~~ — corrected, not parked.**
  ...
  ## Update — Registration & Invite Flow (Post-Approval-Flow Correction)

**Member registration no longer takes invite_code.** With the
organizer-approval flow built (JoinRequest model), joining a group is a
fully separate action from creating an account — there's no more
"auto-join at signup." Registration for Member and Organizer is now
identical in shape; joining any group happens via the invite → preview →
join-request → approval → select-position sequence below, regardless of
whether it's the member's first group or fifth.

## The Full Group-Joining Sequence (as actually built)

1. `POST /groups/:id/invites` — Organizer generates a code
2. `GET /invites/:code` — public, resolves code → group preview
   (includes `group_id`, needed to call the next step)
3. `POST /groups/:id/join-requests` — creates a pending request.
   Rejects if the caller is already a member of this group (409).
4. `GET /join-requests/:id` — the "waiting room" poll. Restricted to
   the requester themselves, or the group's organizer (403 otherwise).
5. `PATCH /join-requests/:id/approve` (or `/reject`) — organizer only
6. `GET /groups/:id/available-slots` — approved requester checks open positions
7. `POST /join-requests/:id/select-position` — creates the real
   GroupMember row. Position must be within `2..totalSlots` (400
   otherwise), and can't collide with an existing member's slot (409,
   distinguished from "you're already a member" via Prisma's err.meta.target)

## `Group.startDate` — Clarified

This is **not a separate concept from "contribution due date"** — it's
literally round 1's due date, just captured at whichever moment is most
natural for the organizer (usually at group creation, since that's when
they're already thinking about it). Can be provided at `POST /groups` or
overridden later at `POST /groups/:id/start-rotation`; if provided in
both places, the start-rotation value wins (it's the more recent,
deliberate decision). If provided nowhere, defaults to "today" — matching
original behavior before this field existed.

**A UI implication worth remembering:** since this is one underlying
fact, a UI showing two separate date pickers ("start date" and "first
contribution due date") is asking the same question twice — worth
flagging to design if that pattern shows up.

## `payoutOrderType` — Renamed, Meaning Clarified

`fixed`/`rotating` → `self_selected`/`organizer_assigned`. Every group
currently behaves as `self_selected` — members choose their own available
slot via the join-request flow (matches the "different people have
different savings urgency" reasoning from product research).
`organizer_assigned` is reserved but has **no implemented behavior** —
selecting it currently changes nothing. Revisit if a future requirement
needs the organizer to pre-assign positions rather than let members choose.

## `ContributionCycle.status: "active"` — What It Actually Means

All cycles generated by `start-rotation` are set to `active` simultaneously
at creation — this is by design, not a bug. `active` means "generated,
not yet completed," not "currently the group's in-progress round." A
member can technically pay round 3 before round 1's payout has happened.
For anything needing "what's the current round," use the derived
`getCurrentCycle()` helper (lowest-numbered non-completed cycle) rather
than assuming only one cycle is ever `active` at a time — this matters
directly for Module D's dashboard work.

## Security Fixes From E2E Testing Round

- `GET /groups/:id/schedule`, `/payout-order`, `/payouts` now all require
  the caller to be an actual member of that group — previously any
  authenticated user could view any group's data.
- `passwordHash` was leaking in every nested `user` object across these
  same endpoints. Fixed via `src/utils/sanitizeUser.js`'s
  `stripPasswordHash` — recursively strips the field from any plain
  object/array, while correctly leaving Dates/Decimals untouched (an
  earlier version of this utility destructively cloned those types,
  losing their prototype methods — fixed via an `isPlainObject` check
  before spreading).

## Parked Decisions — Additions

- **Pagination** on schedule/dashboard-style endpoints — real concern
  once groups/cycles scale up, not needed at MVP data volumes.
- **Invite delivery (SMS/phone)** — `Invite.invitedPhoneNumber` field
  exists, no send mechanism built. Organizer shares codes manually.
- **"Submitted at" precision** for the payment-status UI screen — current
  `GET /contributions/:id` doesn't expose the exact submission timestamp
  (would need to read the first `pending_confirmation` entry off
  `ContributionStatusLog`). Add if the UI needs that specific detail.