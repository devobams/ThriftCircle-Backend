...
export const handleCreateJoinRequest = asyncHandler(async (req, res) => {
  ...
  if (result.invalid) {
    return res
      .status(400)
      .json({ message: `Invite code ${result.reason.replace("_", " ")}` });
  }
  ...
  if (result.alreadyRequested) {
    return res
      .status(409)
      .json({ message: "You already have a pending request for this group" });
  }

  res.status(202).json({
    message: "Join request submitted, pending organizer approval",
    join_request_id: result.joinRequest.id,
  });
});
...
export const handleListPendingJoinRequests = asyncHandler(async (req, res) => {
  ...
  if (result.forbidden)
    return res.status(403).json({ message: "You do not own this group" });

  res.status(200).json(result.requests);
});

export const handleGetJoinRequestStatus = asyncHandler(async (req, res) => {
  const { id } = joinRequestIdParamSchema.parse(req.params);
  const result = await getJoinRequestStatus(id, req.user.id);
  if (result.notFound)
...
export const handleGetAvailableSlots = asyncHandler(async (req, res) => {
  ...
  if (result.forbidden)
    return res
      .status(403)
      .json({ message: "You are not approved to view this group's slots" });
...
export const handleSelectPosition = asyncHandler(async (req, res) => {
  ...
  if (result.notFound)
    return res.status(404).json({ message: "Join request not found" });
  if (result.forbidden)
    return res.status(403).json({ message: "This is not your join request" });
  if (result.notApproved)
    return res
      .status(400)
      .json({ message: "This request has not been approved yet" });
  if (result.invalidPosition)
    return res
      .status(400)
      .json({ message: "That position doesn't exist in this group" });
  if (result.positionTaken)
    return res
      .status(409)
      .json({ message: "This position was just taken, please pick another" });
  if (result.alreadyMember)
    return res
      .status(409)
  ...
  res
    .status(201)
    .json({ message: "Joined successfully", position: result.position });
});
